...
int ClientStub::SendCommitAbort(int final_decision) {
	int signal;
	char buffer[8];
	int net_identify = htonl(TM_RM_IDENTIFY);
	int net_final_decision = htonl(final_decision);
	int offset = 0;
	int size;
	memcpy(buffer + offset, &net_identify, sizeof(net_identify));
	offset += sizeof(net_identify);
	memcpy(buffer + offset, &net_final_decision, sizeof(net_final_decision));
	size = sizeof(net_identify) + sizeof(net_final_decision);
	if (socket.Send(buffer, size, 0)) {
		size = sizeof(int);
		if (socket.Recv(buffer, size, 0)) {
			int net_signal;
			memcpy(&net_signal, buffer, sizeof(net_signal));
			signal = ntohl(net_signal);
		}
	}
	return signal;
}
